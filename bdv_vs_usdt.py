...
def _config_path() -> str:
    """Devuelve la ruta absoluta del archivo de configuración JSON."""
    base_dir = os.path.dirname(os.path.abspath(__file__))
    return os.path.join(base_dir, CONFIG_FILENAME)


def load_config():
    """Carga la configuración persistente desde el archivo JSON."""
    defaults = {"pay_type": "BANK", "amount": 10000.0}
    path = _config_path()
    if os.path.exists(path):
        try:
            ...
            print(f"Advertencia: no se pudo leer la configuración: {e}")
    return defaults
...
def listar_metodos_pago(amount="10000"):
    """
    Obtiene y muestra los métodos de pago disponibles en los anuncios actuales de Binance P2P.
    """
    try:
        ...
        print(
            f"Obteniendo métodos de pago desde Binance P2P (monto ref: {amount} Bs.)..."
        )
        ...
        if "data" in data:
            ...
            for ad in data["data"]:
                for method in ad.get("adv", {}).get("tradeMethods", []):
                    ...
                    if identifier:
                        metodos[identifier] = name

            print("\nMétodos de pago disponibles (identificador: nombre):")
            print("-" * 50)
            for ident, name in sorted(metodos.items()):
                print(f"{ident}: {name}")
            print("-" * 50)
            print("\nPodés usar el identificador con el parámetro --pay-type")
        else:
            print("No se encontraron anuncios para listar métodos de pago.")

    except Exception as e:
        print(f"Error al listar métodos de pago: {e}")


def obtener_tasa_promedio_binance_p2p(pay_type=None, amount="10000"):
    """
    ...
    try:
        ...
        print(
            f"Conectando con Binance P2P{' (filtro: ' + pay_type + ')' if pay_type else ''} (monto ref: {amount} Bs.)..."
        )
        response = requests.post(url, json=payload, headers=headers, timeout=10)
        response.raise_for_status()
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Analiza tasas USD/VES (BCV vs Binance P2P) con cache diario en SQLite"
    )
    parser.add_argument(
        "--no-cache",
        action="store_true",
        help="Ignora el cache del día y fuerza nuevas consultas",
    )
    parser.add_argument(
        "--convert-usd",
        type=float,
        default=None,
        help="Convierte el monto dado en USDT a bolívares usando la(s) tasa(s) disponibles",
    )
    parser.add_argument(
        "--convert-bs",
        type=float,
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help=f"Filtrar por método de pago en Binance P2P (ej: BANK, Banesco). Actual: {config['pay_type']}",
    )
    parser.add_argument(
        "--amount",
        type=float,
        default=None,
        help=f"Monto de referencia en bolívares. Actual: {config['amount']}",
    )
    parser.add_argument(
        "--list-pay-types",
        action="store_true",
        help="Lista los métodos de pago disponibles actualmente en los primeros anuncios de Binance P2P",
    )
    parser.add_argument(
        "--save-config",
        action="store_true",
        help="Guarda los filtros pasados por comando como configuración por defecto para futuras ejecuciones",
    )
    ...
    updated = False
    if args.pay_type:
        config["pay_type"] = args.pay_type
        updated = True
    if args.amount:
        config["amount"] = args.amount
        updated = True

    # Persistir si se pidió explícitamente o si es la primera vez (opcional, vamos a ser explícitos)
    if args.save_config and updated:
        save_config(config)
        print(f"Configuración guardada: {config}")

    if args.list_pay_types:
        listar_metodos_pago(amount=config["amount"])
        return

    print("=== ANÁLISIS DE TASAS DE CAMBIO USD/VES ===")
    print(f"Configuración activa: {config['pay_type']} | {config['amount']} Bs.")
    ...
    if not args.no_cache:
        ...
        if cached:
            print(
                "Usando cache del día (registro más reciente). Use --no-cache para forzar actualización."
            )
            ...
            if args.convert_usd is not None and t_bcv:
                ...
                print("\n— Conversión (cache) —")
                print(
                    f"{monto:.2f} USDT a tasa BCV {t_bcv:.2f} -> {monto_bcv:,.2f} Bs."
                )
                if t_bin:
                    ...
                    print(
                        f"{monto:.2f} USDT a tasa Binance {t_bin:.2f} -> {monto_bin:,.2f} Bs. (Δ {dif:,.2f} Bs., {pct:.2f}%)"
                    )
            ...
            if args.convert_bs is not None:
                ...
                if t_bcv:
                    ...
                    if usdt_bcv is not None:
                        print(
                            f"{monto_bs:,.2f} Bs. a USDT @BCV {t_bcv:.2f} -> {usdt_bcv:,.4f} USDT"
                        )
                else:
                ...
                if t_bin:
                    ...
                    if usdt_bin is not None:
                        print(
                            f"{monto_bs:,.2f} Bs. a USDT @Binance {t_bin:.2f} -> {usdt_bin:,.4f} USDT"
                        )
            ...
            if args.convert_usdt_bcv is not None:
                ...
                if not t_bin:
                    print(
                        "No hay tasa Binance en cache para calcular USDT->VES->USD(BCV)."
                    )
                if t_bcv and t_bin:
                    ves = monto * t_bin
                    ...
                    print("\n— Conversión (cache) —")
                    print(
                        f"{monto:.2f} USDT a VES @Binance {t_bin:.2f} -> {ves:,.2f} Bs."
                    )
                    print(
                        f"{ves:,.2f} Bs. a USD @BCV {t_bcv:.2f} -> {usd_bcv:,.2f} USD (equivalente a tasa BCV)"
                    )
            ...
            if args.convert_bcv_usdt is not None:
                ...
                if not t_bin:
                    print(
                        "No hay tasa Binance en cache para calcular USD(BCV)->VES->USDT."
                    )
                if t_bcv and t_bin:
                    ves = monto_usd * t_bcv
                    ...
                    print(
                        f"{ves:,.2f} Bs. a USDT @Binance {t_bin:.2f} -> {usdt:,.4f} USDT"
                    )
            return
    ...
    datos_binance = obtener_tasa_promedio_binance_p2p(
        pay_type=config["pay_type"], amount=config["amount"]
    )
    ...
    if tasa_bcv and datos_binance:
        ...
        resultados = {
            ...
            "porcentaje_diferencia": ((datos_binance["promedio"] - tasa_bcv) / tasa_bcv)
            * 100,
        }
    ...
    if args.convert_usd is not None:
        ...
        if datos_binance and datos_binance.get("promedio"):
            ...
            if tasa_bcv:
                ...
                print(
                    f"{monto:.2f} USDT a tasa Binance {t_bin:.2f} -> {monto_bin:,.2f} Bs."
                )
    ...
    if args.convert_bs is not None:
        ...
        if tasa_bcv:
            ...
            if usdt_bcv is not None:
                print(
                    f"{monto_bs:,.2f} Bs. a USDT @BCV {tasa_bcv:.2f} -> {usdt_bcv:,.4f} USDT"
                )
        else:
    ...
    if args.convert_usdt_bcv is not None:
        ...
        if tasa_bcv and t_bin:
            ...
            print(
                f"{ves:,.2f} Bs. a USD @BCV {tasa_bcv:.2f} -> {usd_bcv:,.2f} USD (equivalente a tasa BCV)"
            )
